...
class VolumeControl extends React.Component {
  ...
  render () {
    return (
      <div>
        <button className='volume-display' disabled><i className={`fas fa-volume${this.state.volume === 0 ? '-off' : this.state.volume <= 33 ? '-down' : this.state.volume <= 66 ? '' : '-up'}`} /> {this.state.volume}</button>
