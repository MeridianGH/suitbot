...
export function startDashboard(client) {
  ...
  app.get('/callback', async (req, res) => {
    ...
    const token = await client.rest.post(Routes.oauth2TokenExchange(), { body: body, headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, auth: false, passThroughBody: true }).catch((e) => { logging.warn('Error while fetching token while authenticating: ' + e) })
