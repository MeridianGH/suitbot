...
module.exports = {
  simpleEmbed: function (content, ephemeral = false) {
    return { embeds: [new MessageEmbed().setDescription(content).setFooter({ text: 'SuitBot', iconURL: require('./events/ready').iconURL })], ephemeral: ephemeral }
  },
  errorEmbed: function (title, content, ephemeral = false) {
    return { embeds: [new MessageEmbed().setTitle(title).setDescription(content).setFooter({ text: 'SuitBot', iconURL: require('./events/ready').iconURL }).setColor('#ff0000')], ephemeral: ephemeral }
  },
